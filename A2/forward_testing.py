pyDatalog.create_terms('RouteHasStop, DirectRoute, CanReachWithExactTransfer, X, Y, Z, R, R1, R2, Stops')
...
CanReachWithExactTransfer(X, Y, Z, R1, R2) <= (
    DirectRoute(X, Z, R1) & DirectRoute(Z, Y, R2) & (R1 != R2) & (Z == stop_id_to_include)
)

# Forward chaining for optimal route planning with strict transfer at specified stop
def forward_chaining(start_stop_id, end_stop_id, stop_id_to_include, max_transfers):
    ...
    # Find possible paths with a transfer exactly at the specified via stop
    valid_paths = pyDatalog.ask(f"CanReachWithExactTransfer({start_stop_id}, {end_stop_id}, {stop_id_to_include}, R1, R2)")

    # Process results with strict transfer requirements
